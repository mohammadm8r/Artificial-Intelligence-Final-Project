...
public class OthelloGame extends AbstractGame {
    ...
    public int testing_end_game(int[][] board, int markBoard) {
        ...
        //markBoard is the symbol number for our player
        //end = 1 means our player has won
        if (noSpace(board)) {
            //if the board is full, find the winner
        ...
        //count both of the players' marks
        ...
        //if no one can do any valid move find the winner
    ...
    public List<Move> getValidMoves(int[][] board, int markBoard) {
        ...
        for (int i = 0; i < sizeI; i++) {
            for (int j = 0; j < sizeI; j++) {
                BoardSquare movement = new BoardSquare(i, j);
                if (validate_movement(board, movement, othelloPlayer) == 0) {
                    boardPlaces.add(movement);
                }
            }
        }
    ...
    @Override
    public int validate_movement(int[][] board, BoardSquare boardPlace, AbstractPlayer abstractPlayer) {
...
//                check second diagonal -
    ...
    //checks if the inputs (row and column) are one of the 8 places around a specific spot
...
//        skip play
    ...
    //if one of them is empty it returns false
